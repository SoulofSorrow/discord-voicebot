...
export default {
  ...
  async execute(interaction, client, config) {
    try {
      ...
      // Rate limiting
      const rateLimitCheck = rateLimiter.checkLimit(member.id, 'unblock', 10, 60000);
      ...
      const isBlocked = currentPerms?.deny?.has('ViewChannel');
      
      if (!isBlocked) {
        return interaction.reply({
          content: t('user_not_blocked', lang, { user: `<@${target.id}>` }),
          flags: MessageFlags.Ephemeral
        });
      }

      // Remove block permissions (reset to default)
      ...
      metrics.recordInteraction('unblock_user');

      try {
        await interaction.update({
          content: t('unblocked', lang, { user: `<@${target.id}>` }),
          components: []
        });
      } catch (updateError) {
      ...
      await ErrorHandler.handle(error, interaction, client, 'modal:unblock');
    } finally {
